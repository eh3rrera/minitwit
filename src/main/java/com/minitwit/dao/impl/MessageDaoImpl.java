...
public class MessageDaoImpl implements MessageDao {
	...
	public List<Message> getUserTimelineMessages(User user) {
		...
		String sql = "SELECT message.*, user.* FROM message, user WHERE user.user_id = message.author_id AND user.user_id = :id ORDER BY message.pub_date DESC";
	...
	public List<Message> getUserFullTimelineMessages(User user) {
		Map<String, Object> params = new HashMap<String, Object>();
    params.put("id", user.getUserId());
		String sql = "SELECT message.*, user.* FROM message, user WHERE message.author_id = user.user_id AND (user.user_id = :id OR user.user_id IN (SELECT followee_id FROM follower WHERE follower_id = :id)) ORDER BY message.pub_date DESC";
		List<Message> result = template.query(sql, params, messageMapper);
	...
	public List<Message> getPublicTimelineMessages() {
		Map<String, Object> params = new HashMap<String, Object>();
		String sql = "SELECT message.*, user.* FROM message, user WHERE message.author_id = user.user_id ORDER BY message.pub_date DESC";
		List<Message> result = template.query(sql, params, messageMapper);
